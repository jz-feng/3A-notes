...
### Can't be done using $2 \times 2$ matrix
...
\[
    \begin{bmatrix} x' \\ y' \end{bmatrix}
    =
    \begin{bmatrix} x\cos(\Theta) & - y\sin(\Theta) \\ x\sin(\Theta) & y\cos(\Theta) \end{bmatrix}
    \begin{bmatrix} x \\ y \end{bmatrix}
\]
## Solution for translation: \un{homogeneous coordinates}
### $[x, y, w]$ represents a point at $[x/w, y/w]$; i.e. a scaling factor for coordinates
## \un{Order of operations}: scale $\rightarrow$ rotate $\rightarrow$ translate
...
## e.g. mouse event coordinates must be transformed into a model's local coordinates
## Transforming \un{mouse $\rightarrow$ model} coordinates
### Only one transformation -- take the inverse of model's affine matrix
